 ...
 * @constructor
 */
function TestServiceProvider(serviceStubs) {
	this.stubs = serviceStubs || {};
}

TestServiceProvider.prototype = {
	...
	getService: function(serviceName) {
		...
		preRegisteredService = applicationStub.getServiceForTest(serviceName, this);
...
for (var i = 0, len = APPLICATION_CONTEXT_STUBS.length; i < len; i++) {
	...
	TestServiceProvider.prototype[stubName] = functionStub(stubName);
}
