...
(function() {
	...
	Box.TestServiceProvider = function(serviceStubs, allowedServicesList) {
		...
		// Make lookup fast, convert to a map
		var allowedServices = allowedServicesList || [];
		for (var i = 0, len = allowedServices.length; i < len; i++) {
			this.allowedServices[allowedServices[i]] = true;
		}
	};
	...
	Box.TestServiceProvider.prototype = {
		...
		getService: function(serviceName) {
			...
			if (this.allowedServices.hasOwnProperty(serviceName)) {
				...
				throw new Error('Service "' + serviceName + '" is not on the `allowedServiceList`.');
			}
		},
		...
		hasService: function(serviceName) {
			return this.stubs.hasOwnProperty(serviceName) || this.allowedServices.hasOwnProperty(serviceName) && application.hasService(serviceName);
		},
